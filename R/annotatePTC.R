	...
	for(ii in 1:length(uniqueIsoformIds))
	{
		...
		if (currentExonRows[1,"strand"] == "+")
		{
            ...
            if (!is.null(qualifiedCDS)) 
            {
                for (j in 1:nrow(currentExonRows))
                {
                    ...
                    if (nrow(result))
                    {
                        cdsPosGenomic[i] <- min(result$"cdsStart")
                        break
                    }
                }
            }
		...
		if (cdsPosGenomic[i][1] != -1)
		{
			...
			if (currentExonRows[1,"strand"] == "+")
			{
				...
				cdsOffset		<- cdsPosGenomic[i][1]+1 - currentExonRows[cdsExon,"start"] + exonCumsum[cdsExon]+1
			    ...
			    stopPosGenomic[i] <- currentExonRows$start[stopIsLocatedInExon] + distanceToLocalExonstart -3 # -3 since the *3 to get the stopPosTranscipt brings us to the end of the stop codon
			...
			} else if (currentExonRows[1,"strand"] == "-")
			{
				...
				cdsPosExon[i]                       <- cdsExon
				cdsOffset							<- currentExonRows[cdsExon,"end"] -  cdsPosGenomic[i][1]+1 + exonRevCumsum[length(exonRevCumsum)-cdsExon] +1
				...
				stopPosTranscipt[i]							<- (stopPosTranscipt[i]*3)+cdsOffset-1
                ...
                stopPosGenomic[i] <- currentExonRows$end[stopIsLocatedInExon] + distanceToLocalExonstart -2 # -2 since the *3 to get the stopPosTranscipt brings us to the end of the stop codon and then add 1 to the stand
